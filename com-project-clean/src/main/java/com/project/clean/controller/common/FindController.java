		...
		if(adminOrEmplyoee.equals("admin")) {
			...
			if(null != admin) {
				if(admin.getAdminName().equals(userName)) {
					rttr.addFlashAttribute("userFindId", admin.getAdminId());
					
				}
			} else {rttr.addFlashAttribute("userFindId", "입력하신 정보를 확인하세요.");}
			...
			if(null != emp) {
				if(emp.getEmployeeName().equals(userName)) {
					rttr.addFlashAttribute("userFindId", emp.getEmployeeId());
